-- Fix product ordering in venda curva report
-- Order by: Curva (A, B, C, D) then by valor_vendas DESC within each curve
...
BEGIN
  ...
  -- Paginate by dept_nivel3, but return ALL products within those departments
  RETURN QUERY EXECUTE format('
    WITH dept3_totais AS (
      -- Calculate totals per department level 3
      SELECT 
    ...
    LEFT JOIN %I.departments_level_2 d2 ON d2.departamento_id = d1.pai_level_2_id
    LEFT JOIN %I.departments_level_3 d3 ON d3.departamento_id = d1.pai_level_3_id
    INNER JOIN dept3_totais dt ON COALESCE(d3.descricao, ''Sem Nível 3'') = dt.dept3_nome
    WHERE EXTRACT(MONTH FROM v.data_venda) = $1
      AND EXTRACT(YEAR FROM v.data_venda) = $2
      AND v.valor_vendas > 0
      AND p.ativo = true
