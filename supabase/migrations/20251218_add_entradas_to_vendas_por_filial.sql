-- =====================================================
-- Migration: Add Total Entradas to get_vendas_por_filial
-- Created: 2025-12-18
-- Description: Adiciona coluna de Total de Entradas (compras) por filial
--              Filtra transacao IN ('P', 'V') da tabela entradas
--              Agrupa por filial_id usando data_entrada
-- =====================================================

-- Drop função existente
DROP FUNCTION IF EXISTS public.get_vendas_por_filial(TEXT, DATE, DATE, TEXT, TEXT);

-- Criar nova versão com Total de Entradas
...
RETURNS TABLE (
  ...
  -- NOVOS CAMPOS: Total de Entradas
...
BEGIN
  ...
  RETURN QUERY EXECUTE format('
    ...
    -- NOVO: Entradas (compras) - transacao P ou V
    entradas AS (
      SELECT
        filial_id,
        data_entrada,
    ...
    -- NOVO: Entradas período anterior
    ...
    -- NOVO: Combinar todas as filiais de todas as fontes
    todas_filiais AS (
...
Atualizado em 2025-12-18: Adicionado Total de Entradas (compras).

Novos campos:
  - total_entradas: Soma de valor_total da tabela entradas (transacao P ou V)
  - pa_total_entradas: Total de entradas do período anterior
  - delta_entradas: Diferença entre períodos
  - delta_entradas_percent: Variação percentual
