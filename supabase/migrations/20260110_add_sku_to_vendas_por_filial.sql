-- =====================================================
-- Migration: Add SKU to get_vendas_por_filial
-- Created: 2026-01-10
-- Description: Adiciona coluna de Total de SKU (produtos distintos) por filial
--              Usa tabela vendas_hoje_itens com COUNT(DISTINCT produto_id)
--              Filtra apenas itens não cancelados (cancelado = false)
-- =====================================================

-- Drop função existente
DROP FUNCTION IF EXISTS public.get_vendas_por_filial(TEXT, DATE, DATE, TEXT, TEXT);

-- Criar nova versão com Total de SKU
...
BEGIN
  ...
  RETURN QUERY EXECUTE format('
    ...
    -- NOVO: Itens (produtos vendidos - SKU)
    itens AS (
      SELECT
        filial_id,
        data_extracao,
    ...
    -- NOVO: SKU período atual
    sku_periodo_atual AS (
      SELECT
        i.filial_id,
        COUNT(DISTINCT i.produto_id) as total_sku
      FROM itens i
      WHERE i.data_extracao BETWEEN $1 AND $2
    ...
    -- NOVO: SKU período anterior
    sku_periodo_anterior AS (
      SELECT
        i.filial_id,
        COUNT(DISTINCT i.produto_id) as pa_total_sku
      FROM itens i
      WHERE i.data_extracao BETWEEN $3 AND $4
      GROUP BY i.filial_id
    ),
...
COMMENT ON FUNCTION public.get_vendas_por_filial IS 
  'Retorna vendas por filial com dados de PDV, entradas, cupons, SKU e comparação com período anterior. Versão 2026-01-10 com suporte a SKU distintos.';
