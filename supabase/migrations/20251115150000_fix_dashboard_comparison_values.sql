-- =====================================================
-- Migration: Fix Dashboard Comparison Values
-- Created: 2025-11-15 15:00
-- Description: Fix the pa_* fields to return correct comparison values
--              When full year: pa_* should contain full previous year (PAA)
--              When other periods: pa_* should contain previous month/period (PAM)
-- =====================================================
...
DECLARE
  ...
  -- NEW: Variables to hold the final comparison values
...
BEGIN
  ...
  -- ==========================================
  -- DECIDE WHICH COMPARISON TO RETURN (NEW LOGIC)
  -- ==========================================
  IF v_is_full_year THEN
  ...
  ELSE
    -- For other periods: return PAM (previous month) in pa_* fields
  ...
  RETURN QUERY SELECT
    ...
    v_ticket_medio,
    v_margem_lucro,
    v_final_pa_vendas,        -- CHANGED: Now returns correct comparison
    v_final_pa_lucro,          -- CHANGED: Now returns correct comparison
    v_final_pa_ticket_medio,   -- CHANGED: Now returns correct comparison
    v_final_pa_margem_lucro,   -- CHANGED: Now returns correct comparison
...
'Dashboard data with intelligent comparison: full year returns PAA in pa_* fields, other periods return PAM';
