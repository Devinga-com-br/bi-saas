-- Migration: Fix venda curva function - final
-- Data: 2025-10-17
-- Descrição: Corrige problemas de colunas e lógica da função
...
BEGIN
  ...
  v_query := format('
    ...
    SELECT jsonb_build_object(
      ...
      ''page'', %L,
      ''page_size'', %L,
      ''total_pages'', COALESCE(CEIL(MAX(total_count)::NUMERIC / %L), 0),
      ''data'', COALESCE(
        jsonb_agg(
          jsonb_build_object(
