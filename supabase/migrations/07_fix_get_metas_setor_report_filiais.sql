-- ============================================================================
-- FIX: Corrigir referência à tabela filiais em get_metas_setor_report_optimized
-- ============================================================================
-- PROBLEMA: A função está tentando fazer JOIN com schema.filiais, mas a tabela
--           não existe. As filiais estão em public.branches.
--
-- SOLUÇÃO: Trocar %I.filiais por public.branches
-- ============================================================================
...
BEGIN
  ...
  -- ✅ FIX: Trocar %I.filiais por public.branches (linha 102)
  EXECUTE format('
        ...
        ''filiais'', (
          SELECT json_agg(
            json_build_object(
              ...
              ''filial_nome'', COALESCE(b.name, ''Filial '' || msf.filial_id),
              ...
              ''percentual_atingido'', CASE
                ...
                ELSE 0
              END
            ) ORDER BY COALESCE(b.name, ''Filial '' || msf.filial_id)
          )
          FROM %I.metas_setor msf
          LEFT JOIN public.branches b ON CAST(b.code AS BIGINT) = msf.filial_id AND b.tenant_id = (SELECT id FROM public.tenants WHERE supabase_schema = %L LIMIT 1)
...
'Retorna relatório de metas por setor para um mês específico (VERSÃO OTIMIZADA - FIXED).

FIX: Corrigido JOIN com public.branches ao invés de schema.filiais que não existe.
