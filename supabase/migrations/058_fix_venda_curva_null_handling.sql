-- Fix NULL handling in venda curva report
...
BEGIN
  -- Handle NULL filial_id
  IF p_filial_id IS NULL THEN
    v_filial_filter := 'TRUE';
  ELSE
    v_filial_filter := format('v.filial_id = %s', p_filial_id);
  END IF;
  ...
  $query$,
    ...
    v_filial_filter,  -- Filial filter (dynamic)
